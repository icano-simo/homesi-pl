...
-- ═══════════════════════════════════════════════════════════════════════════
--
-- APPLIED to production on 2026-08-27, and verified there: the table exists in
-- finance_division with RLS on and no policies, privileges only for
-- service_role, nothing created in public, and the PostgREST cache reloaded.
-- It is EMPTY — the two rows it anticipates are still commented out below,
-- because deciding that two names are one person is not a migration's call.
-- Do not run it again.
--
-- NOTHING READS IT YET. It was written for the Loan Officer P&L, which was
-- shelved on 2026-08-27 and lives on feature/lo-pnl-and-loan-fixes. The table
-- stays because dropping and recreating it would cost more than leaving an
-- empty one in place.
--
-- AND IT MAY NEVER BE READ. Compensafe is coming from BigQuery, where
-- comp_marts already resolves names through person_code and
-- hr_centralizado.person_name_key — which is precisely what this table does by
-- hand. If that lands, this becomes dead weight and should be dropped in its
-- own migration. Not decided yet; do not pre-empt it.
--
-- This file reached main on its own, after the module it belongs to did not:
-- the repository has to describe the schema that exists, and the table does.
--
-- WHAT THIS IS FOR
--
