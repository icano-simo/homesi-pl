...
import type { NoteLevel } from "@/lib/note-scope";
...
interface DescGroup { key: string; label: string; total: number; count: number }
...
interface CellDetail {
  ...
  row_count: number;
  total: number;
  levels: Record<DescField, NoteLevel>;
  coverage: Record<DescField, number>;
  groups: Record<DescField, DescGroup[]>;
}
 ...
 *
 * The window shows the figure that was clicked and the rows immediately beneath
 * it — a cost centre broken into categories, a category into accounts, an
 * account into descriptions. It never goes deeper, which is what makes every
 * level cost the same to open and why a cost centre no longer drags its whole
...
export function CellDetailModal({
  ...
  /**
   * Which description to group by. Null until the reader picks: preselecting
   * would hide that the right choice is account-dependent, which is the whole
  ...
  /** Only the deepest level needs the server; every other breakdown is already
   *  in the tree the table built. */
  const params = useMemo(() => {
    if (!target || target.children) return null;
    const s = target.scope;
    if (!s.gl && !s.category_6 && !s.category_7 && !s.cost_center) return null;
    ...
    // Only when the cell is not already pinned to one centre.
    if (!s.cost_center) target.costCenterIds.forEach((c) => p.append("cost_center_id", c));
    return p.toString();
  }, [target]);

  // A new cell is a new choice, in both selectors.
  const cellKey = target ? JSON.stringify(target.scope) : "";
  useEffect(() => {
    setField(null); setAnchorKey(""); setDraft(""); setSaveError("");
  }, [cellKey]);

  useEffect(() => {
    if (!params) { setData(null); setLoading(false); setError(""); return; }
    let cancelled = false;
  ...
  /**
   * The breakdown, whichever side produced it — and with it the anchors on
   * offer, because they are the same rows. A note is either about this cell or
   * about one of the lines under it.
   */
  const breakdown = useMemo<BreakdownRow[] | null>(() => {
    ...
    return data.groups[field].map((g) => ({
      level:      data.levels[field],
      levelLabel: DESC_LABELS[field],
      valueLabel: g.label,
      scope:      { ...target.scope, [data.levels[field]]: g.key, ...(month ? { month } : {}) },
      amount:     g.total,
      count:      g.count,
    }));
  }, [target, data, field]);
  ...
  const columnLabel = target.children
    ? target.childLevelLabel
    : field ? DESC_LABELS[field] : null;
  // Same shape of gap in both cases, and the window says so rather than letting
  // the reader assume the numbers reconcile.
  const opNonOpCaveat = target.scope.op_nonop != null;
  const splitCaveat   = !target.children && target.scope.cost_center != null;
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3 border-b border-slate-200 px-5 py-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 flex items-baseline gap-2">
              <span className={`font-mono text-2xl font-bold tabular-nums ${target.amount < 0 ? "text-rose-600" : "text-[#001A40]"}`}>
                {fmt(target.amount)}
              </span>
              <span className="text-[11px] text-slate-500">
                {/* The Total column is a period of its own — every month on
                    screen — and a note written here carries no month. */}
                {target.month ?? "all months shown"}
              </span>
            </p>
          </div>
        ...
        {/* ── Which description, when the breakdown is by description ─────── */}
        {!target.children && (
          <div className="border-b border-slate-200 px-5 py-3">
            ...
            {opNonOpCaveat && (
              <p className="mt-2 text-[10px] text-amber-700">
                The Operational / Non-Operational split is applied by the report, not stored on the
                rows — this breakdown is the whole cell, before that split.
              </p>
            )}
            {splitCaveat && (
              <p className="mt-2 text-[10px] text-amber-700">
                Grouped by the cost centre each row is assigned to. A transaction shared between
                centres counts here in full, not at its allocated share.
              </p>
            )}
          </div>
        )}
        ...
        <div className="flex-1 overflow-auto">
          ...
          {!loading && !error && !breakdown && (
            <p className="p-5 text-sm text-slate-500">
              {target.children === null && !field
                ? <>Pick a description to break these {data?.row_count ?? 0} movements down by.
            ...
            <p className="min-w-0 truncate text-[10px] text-slate-400">
              {anchor
                ? <>Anchored to {anchor.levelLabel} · {anchor.valueLabel}
                    {target.month ? ` · ${target.month}` : " · all months shown"} ·{" "}
